...
import { BarChart3, TrendingUp, Users, Target, Download, Zap, AlertCircle, Loader2 } from "lucide-react";
...
function Analytics() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const computed = sessionList.map((s, i) => {
          const data = results[i].status === "fulfilled" ? results[i].value?.data : null;
          const records = data?.attendance_records || data?.records || [];
          const present = records.filter((r) => r.status === "present" || r.check_in_method).length;
  ...
  const avgAttendance = totalRecords > 0 ? `${Math.round((totalPresent / totalRecords) * 100)}%` : "—";

  const statCards = [
    { label: "Avg Attendance", val: loading ? "..." : avgAttendance, sub: "Across all sessions", icon: TrendingUp, color: "text-emerald-400", bg: "bg-emerald-500/10" },
    { label: "Total Sessions", val: loading ? "..." : String(sessions.length), sub: "All time", icon: Target, color: "text-indigo-400", bg: "bg-indigo-500/10" },
    { label: "Total Check-ins", val: loading ? "..." : String(totalPresent), sub: "Verified attendances", icon: Users, color: "text-primary-light", bg: "bg-primary/20" },
    { label: "System Status", val: "Optimal", sub: "All nodes responding", icon: Zap, color: "text-cyan-400", bg: "bg-cyan-500/10" },
  ];

  const handleExport = () => {
    const csv = [
      "Session,Course,Location,Total,Present,Rate",
      ...modules.map((m) => `"${m.label}","${m.course}","${m.location}",${m.total},${m.attended},${m.percent}`),
    ].join("\n");
      ...
      <div className="flex-1 flex flex-col ml-64">
        ...
        <main className="p-10 space-y-12 max-w-[1600px] mx-auto w-full">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 border-b border-slate-800 pb-10">
            <div>
              <h2 className="text-4xl font-black text-white tracking-tighter">System Oversight</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {statCards.map((stat, i) => (
              <div key={i} className="bg-slate-800/50 p-10 rounded-[40px] border border-slate-700/50 shadow-2xl flex flex-col gap-6 group hover:border-primary/50 transition-all duration-500">
                <div className={`p-4 w-fit rounded-[24px] ${stat.bg} ${stat.color} group-hover:scale-110 transition-transform`}>
                  <stat.icon size={28} />
                </div>
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-bold mt-2">{stat.sub}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-slate-800/30 rounded-[48px] shadow-2xl border border-slate-800 overflow-hidden backdrop-blur-sm">
            <div className="px-10 py-10 border-b border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <h3 className="text-xl font-bold text-white tracking-tight">Session Attendance Breakdown</h3>
              </div>
              <div className="flex items-center gap-2 px-4 py-2 bg-slate-900/50 rounded-full border border-slate-800">
                <AlertCircle size={14} className="text-amber-500" />
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Live Data</span>
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 gap-3 opacity-40">
                  <BarChart3 size={40} strokeWidth={1} className="text-slate-500" />
                  <p className="text-sm font-bold text-slate-400">No session data yet</p>
                  <p className="text-xs text-slate-500">Sessions created by teachers will appear here.</p>
                </div>
              ) : (
                      ...
                      <tr key={i} className="hover:bg-slate-800/50 transition-colors group cursor-pointer">
                        <td className="px-10 py-8">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <span className="text-sm font-bold text-slate-200 group-hover:text-primary-light transition-colors block">{row.label}</span>
                              {row.location && <span className="text-[10px] text-slate-500 font-medium">{row.location}</span>}
                            </div>
                          </div>
                        </td>
                        <td className="px-10 py-8 text-xs text-slate-400 font-black">{row.course || "—"}</td>
                        <td className="px-10 py-8 text-center text-xs text-slate-400 font-black">{row.attended}</td>
